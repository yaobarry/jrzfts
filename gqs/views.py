...
def bitwise(object,condition,field,value):
    object_last = fliter(lambda object.field: object.field==object.field & value,object.objects.filter(condition))
    return object_last
...
def paywaylist(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payway_list = bitwise( PayList,'status=1',pid,cd.payway)
            subpayway_list = bitwise(SubPayList,'status=1',pid,cd.subpayway)
